...
clf = load('./model/svm_rbf.joblib')
vector_embedding = en_core_web_md.load()
sentence = "i love this album. it's very good. more to the hip hop side than her current pop sound.. SO HYPE! i listen to this everyday at the gym! i give it star rating all the way. her metaphors are just crazy. Just Awesome".lower()
doc = vector_embedding(sentence)
out = np.asarray(doc.vector).reshape(1, -1)

print(clf.predict(out))
scores = clf.predict_proba(out)[0]
print(scores)
print(0, 1)
print("fsd", max(scores) - min(scores))
'''
